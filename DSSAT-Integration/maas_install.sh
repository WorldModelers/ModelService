...
docker run --rm --volumes-from ethdata -v ${PWD}:/userdata debian:stable-slim bash -c "apt-get update && apt-get install xz-utils && cd /data/base && tar xJvf /userdata/basedata-20191003.tar.xz && cd /data/ETH && tar xJvf /userdata/ethdata-20191003.tar.xz && cd /data/SS && tar xJvf /userdata/ssddata-20191003.tar.xz"
